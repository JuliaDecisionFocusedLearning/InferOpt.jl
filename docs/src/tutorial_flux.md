```@meta
EditURL = "../../examples/tutorial_flux.jl"
```
...
This tutorial uses [Flux.jl](https://github.com/FluxML/Flux.jl) as the neural network framework.
For a Lux.jl version, see the [companion tutorial](@ref "Basic tutorial (Lux)").

````@example tutorial_flux
using Flux
using InferOpt
using LinearAlgebra
using Random
...
opt_state = Flux.setup(Flux.Adam(1e-3), encoder)
losses = Float64[]
for epoch in 1:100
    l = 0.0
