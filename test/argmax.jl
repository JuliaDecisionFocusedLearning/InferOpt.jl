@testitem "Argmax - imitation θ" default_imports = false begin
    using Flux, InferOpt, LinearAlgebra, Test
    ...
    pipeline = (; encoder, maximizer, loss)
    cost(y; instance) = dot(y, -true_encoder(instance))
    error_function(ŷ, y) = hamming_distance(ŷ, y)

    ## Test loop

    pipeline_loss(x, θ, y) = loss(maximizer(encoder(x)), θ)
